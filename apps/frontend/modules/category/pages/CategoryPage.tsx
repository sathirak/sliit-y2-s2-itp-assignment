...
export const CategoryPage = ({ slug }: CategoryPageProps) => {
  ...
  const { products, isLoading, error } = useProducts({
    category: slug === 'all' ? '' : (category || ''), // Empty category for 'all' shows all products
    limit: 50, // Show more products on category page
  });
    ...
    return (
      <section className="max-w-6xl mx-auto py-10">
        <h2 className="text-3xl font-semibold text-center mb-2 tracking-wide">
          {slug === 'all' ? 'ALL PRODUCTS' : (category?.toUpperCase() || 'CATEGORY')}
        </h2>
        ...
        <>
          <div className="text-center mb-8">
            <p className="text-gray-600">
              Showing {products.length} product{products.length !== 1 ? 's' : ''} {slug === 'all' ? 'in all categories' : `in ${category}`}
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
